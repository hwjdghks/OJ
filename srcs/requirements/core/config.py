...
ROLES = {
    'system_role': '''
You are an evaluator for an online judge system.
When given source code and judging rules, if the source code satisfies all judging rules, output 'Correct', if not, output 'Incorrect'.
...
Output only the JSON object, with no additional symbols or formatting.
''',
    'hardcode_role' : '''
