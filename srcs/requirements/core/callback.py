...
'''
    ...
    if exit_code == 10:
        ...
        ai_res = client.chat.completions.create(
            ...
            {
                'role': 'system',
                'content': '너는 차갑고 냉철한 알고리즘 분석가야. 네게 검토해야할 소스 코드와 소스 코드가 적용해야할 알고리즘을 키워드로 주어질거야. 너는 소스 코드가 키워드를 전부 만족하는지 확인하고 yes 또는 no로 대답해야 해. 이 두 단어 외에 부가적인 설명은 전혀 필요 없어. 단, 모든 소스 코드는 하드코딩 되어있지 않아야해. 만약 소스코드가 전반적으로 하드코딩 되어 있다면 무조건 no를 반환해. 문법적으로 잘못됐는지, 실행 중 에러가 발생하는지 등의 여부는 고려하지마. 오직 키워드의 적용여부만 판단해'
            },
            {
                'role': 'user',
