...
async function addUserHandler(req, res) {  
  try {
    ...
    let suffix = 1;
    while (userExists) {
      const [rows] = await pool.query('SELECT COUNT(*) AS count FROM users WHERE user_id = ?', [userId]);
      if (rows[0].count === 0) {
        userExists = false;
      } else {
        userId = `${userIdBase}${suffix}`;
        suffix++;
      }
    }
...
async function getUserHandler(req, res) {
  try {
    ...
    // 이메일을 기반으로 사용자 조회
    const pool = await mysqlConnect();
    const [rows] = await pool.query('SELECT user_id FROM users WHERE user_email = ?', [email]);
    if (rows.length === 0) {
